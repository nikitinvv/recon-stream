...
def run(out, inp, chunk, proper, nonproper, axis_out, axis_inp, cl, func, igpu, ngpus):
    ...
    out_pinned = np.frombuffer(pinned_mem, out.dtype, np.prod([2,*out_shape0])).reshape([2,*out_shape0])
    ...
    for j in range(2): # do it twice to assign memory pointers
        for k in range(proper):     
            ...
            inp_gpu[j].append(cp.ndarray(inp_shape0, dtype=inp[k].dtype, memptr=gpu_mem+offset))
            inp_pinned[j].append(np.frombuffer(pinned_mem+offset, inp[k].dtype, np.prod(inp_shape0)).reshape(inp_shape0))
        ...
        if (k < nchunk):
            with stream[0]:  # copy to pinned memory
                ...
                for j in range(proper):                            
                    if axis_inp==0:
                        copy(inp[j][st:end],inp_pinned[k % 2][j][:end-st], 16//ngpus)
                    elif axis_inp==1:
                        copy(inp[j][:,st:end],inp_pinned[k % 2][j][:,:end-st], 16//ngpus)
